...
const PostCard: FC<Props> = ({ id, title, date, coverImg, description, blurCss, tags }) => {
  return (
    <Link
      href={`/posts/${id}`}
      passHref>
      <div className="cursor-pointer group max-w-[400px] min-w-[300px] h-96 rounded-3xl flex flex-col flex-1 gap-2 relative shadow-xl dark:shadow-lg shadow-slate-500 dark:shadow-black">
        ...
        <section className="z-10 w-full h-full flex flex-col justify-end gap-8 px-10 pb-8 text-slate-50">
          ...
          <div className="text-md w-full line-clamp-2 max-w-2xl">{description}</div>
